...
public:
	 ...
	 * Print the SDL error
	 */
	static void print_SDLNet_error()
	{
	 ...
	 */
	static void print_SDLNet_error_and_exit(int code = 1)
	{
		print_SDLNet_error();
		exit(code);
	}
	 ...
	 */
	static void print_ip(Uint32 ip, bool newline = false)
	{
		std::cout << (ip & 0xFF) << "." //
			<< ((ip >> 8) & 0xFF) << "." //
			<< ((ip >> 16) & 0xFF) << "." //
	...
	static void print_ip(IPaddress ip, bool newline = false)
	{
	 ...
	 */
	static Uint16 getSocketPort(UDPsocket sock)
	{
	 ...
	 */
	static IPaddress* getSocketIP(TCPsocket sock)
	{
	...
	static Uint32 sdlnet_hton(Uint32 v)
	{
		Uint32 nv;
		SDLNet_Write32(v, &nv);
		return nv;
	...
	static Uint32 sdlnet_ntoh(Uint32 nv)
	{
		return SDLNet_Read32(&nv);
	}

	static Uint16 sdlnet_hton(Uint16 v)
	{
		Uint16 nv;
		SDLNet_Write16(v, &nv);
		return nv;
	...
	static Uint16 sdlnet_ntoh(Uint16 nv)
	{
		return SDLNet_Read16(&nv);
	}

	static Uint8 sdlnet_hton(Uint8 v)
	{
		return v;
	}

	static Uint8 sdlnet_ntoh(Uint8 nv)
	{
		return nv;
	}
	...
	static Uint8* serialize_all(Uint8* buf)
	{
		return buf;
	}
	...
	static Uint8* serialize_all(Uint8* buf, T* a, Uint32&& N,
	                            Ts&&... args)
	{
	...
	template <typename T, typename... Ts>
	static Uint8* serialize_all(Uint8* buf, T&& a, Ts&&... args)
	{
		buf = serialize(a, buf);
		return serialize_all(buf, std::forward<Ts>(args)...);
	}
	...
	// the case of no argument
	static Uint8* deserialize_all(Uint8* buf)
	{
		return buf;
	}
	...
	template <typename T, typename... Ts>
	static Uint8* deserialize_all(Uint8* buf, T* a, Uint32&& N,
	                              Ts&&... args)
	{
	...
	// the case of other non-array types
	template <typename T, typename... Ts>
	static Uint8* deserialize_all(Uint8* buf, T&& a, Ts&&... args)
	{
	...
	template <typename T>
	static Uint8* serialize(T& v, Uint8* buf)
	{
		return v.serialize(buf);
	}

	template <typename T>
	static Uint8* deserialize(T& v, Uint8* buf)
	{
	...
	// ASCII codes
	static Uint8* serialize(char& v, Uint8* buf)
	{
		Uint8& aux_v = reinterpret_cast<Uint8&>(v);
		return serialize_uint(aux_v, buf);
	}

	static Uint8* deserialize(char& v, Uint8* buf)
	{
		Uint8& aux_v = reinterpret_cast<Uint8&>(v);
	...
	// case for Uint8
	static Uint8* serialize(Uint8& v, Uint8* buf)
	{
		return serialize_uint(v, buf);
	}

	static Uint8* deserialize(Uint8& v, Uint8* buf)
	{
		return deserialize_uint(v, buf);
	}

	// case for Uint16
	static Uint8* serialize(Uint16& v, Uint8* buf)
	{
		return serialize_uint(v, buf);
	}

	static Uint8* deserialize(Uint16& v, Uint8* buf)
	{
		return deserialize_uint(v, buf);
	}

	// case for Uint32
	static Uint8* serialize(Uint32& v, Uint8* buf)
	{
		return serialize_uint(v, buf);
	}

	static Uint8* deserialize(Uint32& v, Uint8* buf)
	{
		return deserialize_uint(v, buf);
	}

	// case for Sint8
	static Uint8* serialize(Sint8& v, Uint8* buf)
	{
		return serialize_sint<Uint8>(v, buf);
	}

	static Uint8* deserialize(Sint8& v, Uint8* buf)
	{
		return deserialize_sint<Uint8>(v, buf);
	}

	// case for Sint16
	static Uint8* serialize(Sint16& v, Uint8* buf)
	{
		return serialize_sint<Uint16>(v, buf);
	}

	static Uint8* deserialize(Sint16& v, Uint8* buf)
	{
		return deserialize_sint<Uint16>(v, buf);
	}

	// case for Sint32
	static Uint8* serialize(Sint32& v, Uint8* buf)
	{
		return serialize_sint<Uint32>(v, buf);
	}

	static Uint8* deserialize(Sint32& v, Uint8* buf)
	{
	...
	//
	template <typename T>
	static Uint8* serialize_array(T* v, std::size_t N, Uint8* buf)
	{
	...
	template <typename T>
	static Uint8* deserialize_array(T* v, std::size_t N, Uint8* buf)
	{
		static_assert(!std::is_compound_v<T>);
		for (auto i = 0u; i < N; i++)
	...
	static Uint8* serialize_array(char* v, std::size_t N, Uint8* buf)
	{
		return serialize_arraybyte(reinterpret_cast<Uint8*>(v), N, buf);
	}

	static Uint8* deserialize_array(char* v, std::size_t N, Uint8* buf)
	{
		return deserialize_arraybyte(reinterpret_cast<Uint8*>(v), N, buf);
	}

	// case for array of Uint8
	static Uint8* serialize_array(Uint8* v, std::size_t N, Uint8* buf)
	{
		return serialize_arraybyte(v, N, buf);
	}

	static Uint8* deserialize_array(Uint8* v, std::size_t N,
	                                Uint8* buf)
	{
	...
	//
	static bool send(Uint8* v, TCPsocket sock, msgSize_t msgSize)
	{
		...
		result = SDLNet_TCP_Send(sock, &size,
		                         sizeof(msgSize_t));
	...
	//
	static buff_t receiveMsg(TCPsocket sock)
	{
		int result = 0;
	...
	//
	template <typename T>
	static bool serializedSend(T& v, TCPsocket sock)
	{
		...
		auto result = SDLNet_TCP_Send(sock, &size,
		                              sizeof(msgSize_t));
		if (result != sizeof(msgSize_t))
			return false;
	...
	//
	template <typename T>
	static bool deserializedReceive(T& v, TCPsocket sock)
	{
	...
	template <typename T>
	static int serializedSend(T& m, UDPpacket* p, UDPsocket sock)
	{
	...
	template <typename T>
	static int deserializedReceive(T& m, UDPpacket* p, UDPsocket sock)
	{
		int status = 0;
...
private:
	...
	static Uint8* serialize_uint(UnsignedT& v, Uint8* buf)
	{
	...
	template <typename UnsignedT>
	static Uint8* deserialize_uint(UnsignedT& v, Uint8* buf)
	{
	...
	static Uint8* serialize_sint(SignedT& v, Uint8* buf)
	{
		return serialize_uint<UnsignedT>(reinterpret_cast<UnsignedT&>(v), buf);
	}

	template <typename UnsignedT, typename SignedT>
	static Uint8* deserialize_sint(SignedT& v, Uint8* buf)
	{
	...
	static Uint8* serialize_arraybyte(Uint8* v, std::size_t N,
	                                  Uint8* buf)
	{
		memcpy(buf, v, N);
		return buf + N;
	...
	static Uint8* deserialize_arraybyte(Uint8* v, std::size_t N,
	                                    Uint8* buf)
	{
		memcpy(v, buf, N);
		return buf + N;
